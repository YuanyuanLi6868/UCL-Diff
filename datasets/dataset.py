...
class UIEBdataset:
    ...
    def get_loaders(self):
        ...
        train_dataset = UIEBDataset(os.path.join(self.config.data.data_dir, self.config.data.train_dataset, 'train'),
                                          patch_size=self.config.data.patch_size,
                                          filelist='{}_train.txt'.format(self.config.data.train_dataset))
        val_dataset = UIEBDataset(os.path.join(self.config.data.data_dir, self.config.data.val_dataset, 'val'),
                                        patch_size=self.config.data.patch_size,
...
class UIEBDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, filelist=None, train=True):
        ...
        with open(self.train_list) as f:
            ...
            gt_names = [i.strip().replace('low', 'pso') for i in input_names]
