...
class cross_attention(nn.Module):
    def __init__(self, dim, num_heads, dropout=0.):
        ...
        self.query = Depth_conv(in_ch=dim, out_ch=dim)  #这里的Depth_conv是深度可分离卷积
...
class HFRM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_tail = Depth_conv(out_channels, in_channels)   #深度可分离卷积
    ...
    def forward(self, x):
        ...
        x_HL, x_LH, x_HH = x[:b//3, ...], x[b//3:2*b//3, ...], x[2*b//3:, ...]  #依次是H，V，D

        x_HH_LH = self.cross_attention0(x_HH, x_LH)
        x_HH_HL = self.cross_attention1(x_HH, x_HL)
        x_HL = self.dilated_block_HL(x_HH_HL)
        x_LH = self.dilated_block_LH(x_HH_LH)
        x_HH = self.dilated_block_HH(x_HH)
