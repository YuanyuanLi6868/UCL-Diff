...
class SCConv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.sigmoid(
            torch.add(identity, F.interpolate(self.k2(x), identity.size()[2:])))  # sigmoid(identity + k2)  #第一个分支的第一行
        out = torch.mul(self.k3(x), out)  # k3 * sigmoid(identity + k2)  #得到K4
        out = self.k4(out)  # k4
...
class ResidualBlock(nn.Module):  #9个残差块
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=9, opt=None):
        ...
        self.conv_1 = nn.Sequential(
            nn.ReflectionPad2d(3),  #保持输出特征与输入特征的大小
